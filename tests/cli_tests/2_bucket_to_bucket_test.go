...
func TestZs3ServerBucket(testSetup *testing.T) {
	...
	t.RunSequentially("Test for moving file from testbucket to testbucket", func(t *test.SystemTest) {
		...
		cli_utils.RunCommand(t, "mc rm zcn/testbucket2", 1, time.Hour*2)

		cli_utils.RunCommand(t, "mc rm zcn/testbucket", 1, time.Hour*2)
